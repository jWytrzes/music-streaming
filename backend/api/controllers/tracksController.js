...
exports.getAllTracks = async (req, res, next) => {
	try {
		...
		const tracks = await connection
			.getRepository(trackEntity)
			.find({ relations: ['album', 'artist', 'genre', 'playlists'] });
...
exports.getTrack = async (req, res, next) => {
	try {
		...
		const connection = await database.makeConnection();
		const track = await connection.getRepository(trackEntity).findOne({
			relations: ['album', 'artist', 'genre', 'playlists'],
			where: {
				ID: id,
			},
		});
...
exports.addTrack = async (req, res, next) => {
	...
	try {
		...
		res.status(200).json({ status: 200, message: 'Track added' });
	} catch (error) {
		res.status(error.status || 500).json({
			message: error.message,
		});
...
exports.editTrack = async (req, res, next) => {
	try {
		const id = req.params.id;
		const playlistToAdd = req.body.playlistToAdd.playlistID;
		const connection = await database.makeConnection();
		...
		res.status(200).json({ status: 200, message: 'Track updated' });
	} catch (error) {
		res.status(error.status || 500).json({
			message: error.message,
		});
